---
title: Pakete und Programme | System Center Configuration Manager
...
4.  Geben Sie auf der Seite **Paket** des Assistenten zum Erstellen von Paketen und Programmen die folgenden Informationen an:  

    -   **Name** : Geben Sie einen Paketnamen mit maximal 50 Zeichen an.  

    -   **Beschreibung** : Geben Sie bei Bedarf eine Paketbeschreibung mit maximal 128 Zeichen an.  

    -   **Hersteller**: Geben Sie bei Bedarf einen Herstellernamen an, um das Paket in der Configuration Manager-Konsole besser identifizieren zu können. Der Name darf maximal 32 Zeichen umfassen.  

    -   **Sprache** : Geben Sie bei Bedarf die Sprachversion des Pakets mit maximal 32 Zeichen an.  

    -   **Version** : Geben Sie bei Bedarf eine Versionsnummer für das Paket mit maximal 32 Zeichen an.  
    ...
    -   **Quellordner** : Wenn das Paket Quelldateien enthält, klicken Sie auf **Durchsuchen** , um das Dialogfeld **Quellordner festlegen** zu öffnen und dort den Quelldateipfad für das Paket anzugeben.  

        > [!NOTE]  
        >  Das Computerkonto des Standortservers muss über Lesezugriffsberechtigungen für den angegebenen Quellordner verfügen.  

5.  Wählen Sie im Assistenten zum Erstellen von Paketen und Programmen auf der Seite **Programmtyp** den Typ des zu erstellenden Programms aus, und klicken Sie dann auf **Weiter**. Sie können ein Programm für einen Computer oder für ein Gerät erstellen oder diesen Schritt überspringen und später ein Programm erstellen.  

    > [!TIP]  
    >  Zum Erstellen eines neuen Programms für ein vorhandenes Paket wählen Sie das Paket aus, und klicken Sie dann auf der Registerkarte **Startseite** in der Gruppe **Paket** auf **Programm erstellen** , um den Assistenten zum Erstellen von Programmen zu öffnen.  
...
6.  Wenden Sie eines der folgenden Verfahren an, um ein Standardprogramm oder ein Geräteprogramm zu erstellen.  
  ...
  1.  Auf der **Programmtyp** auf der Seite der **Erstellen eines Pakets und Programms Assistenten**, wählen **Standardprogramm**, und klicken Sie dann auf **Weiter**.     2.  Geben Sie auf der Seite **Standardprogramm** folgende Informationen an:  
        ...
        -   **Befehlszeile** : Geben Sie die Befehlszeile zum Starten dieses Programms ein, oder klicken Sie auf **Durchsuchen** , um den Speicherort der Datei zu suchen.  
        ...
        -   **Startordner** : Geben Sie über dieses Feld bei Bedarf den Ordner mit maximal 127 Zeichen an, von dem das Programm ausgeführt wird. Dieser Ordner kann als absoluter Pfad auf dem Client oder relativ zu dem Verteilungspunktordner angegeben sein, in dem das Paket enthalten ist.  

        -   **Ausführen** : Gibt den Modus an, in dem das Programm auf Clientcomputern ausgeführt wird. Wählen Sie eine der folgenden Einstellungen aus:  
            ...
            -   **Hidden** – die Anwendung ausgeführt wird, die auf Clientgeräten ausgeblendet. Benutzer sehen keine Installationsaktivitäten.  

        -   **Programm kann ausgeführt werden:** Geben Sie hier an, ob das Programm nur ausgeführt werden kann, wenn ein Benutzer beim Clientcomputer angemeldet ist, wenn kein Benutzer angemeldet ist, oder ob die Ausführung in beiden Fällen möglich sein soll.  
        ...
        -   **Benutzern gestatten, die Programminstallation anzuzeigen und mit ihr zu interagieren:** Verwenden Sie diese Einstellung, falls verfügbar, um anzugeben, ob der Benutzer mit der Installation des Programms interagieren darf. Dieses Kontrollkästchen steht nur zur Verfügung, wenn für **Programm kann ausgeführt werden** die Option **Nur wenn kein Benutzer angemeldet ist** oder **Unabhängig von Benutzeranmeldung** und für **Ausführmodus** die Option **Mit Administratorrechten ausführen**ausgewählt ist.  
        ...
        -   **Laufwerkmodus:** Geben Sie hier Informationen zur Ausführung des Programms im Netzwerk an. Wählen Sie eine der folgenden Optionen aus:  
            ...
            -   **Erfordert Laufwerkbuchstaben** -gibt an, dass das Programm zur vollständigen Kennzeichnung seines Speicherortes einen Laufwerkbuchstaben erfordert. Für diese Einstellung kann von Configuration Manager jeder verfügbare Laufwerkbuchstabe auf dem Client verwendet werden.  
  ...
  3.  Geben Sie im Assistenten zum Erstellen von Paketen und Programmen auf der Seite **Anforderungen** die folgenden Informationen an:  
        ...
        -   **Geschätzter Speicherplatz:** Geben Sie hier den Speicherplatz an, der zur Ausführung des Softwareprogramms auf dem Computer erforderlich ist. Er kann als **Unbekannt** (die Standardeinstellung) oder als eine ganze Zahl größer oder gleich Null angegeben werden. Wenn ein Wert angegeben ist, müssen auch Einheiten für diesen Wert angegeben werden.  

        -   **Maximal zulässige Laufzeit (Minuten)** : Geben Sie hier an, wie lange die Ausführung des Programms auf dem Clientcomputer voraussichtlich maximal dauert. Er kann als **Unbekannt** (die Standardeinstellung) oder als eine ganze Zahl größer Null angegeben werden.  

             Standardmäßig ist dieser Wert auf 120 Minuten festgelegt.  

            > [!IMPORTANT]  
            >  Wenn Sie bei der Sammlung, für die dieses Programm ausgeführt wird, Wartungsfenster verwenden, kann ein Konflikt auftreten, wenn die **Maximal zulässige Laufzeit** länger ist als das geplante Wartungsfenster. Jedoch, wenn die maximale Laufzeit ist festgelegt **unbekannten**, das Programm während des Wartungsfensters ausgeführt und wird weiterhin ausgeführt, je nach Bedarf nach dem Schließen des Wartungsfensters. Wenn der Benutzer die maximal zulässige Laufzeit auf einen bestimmten Zeitraum, der die Länge der verfügbaren Wartungsfenster überschreitet wird, wird das Programm nicht ausgeführt werden.  
  ...
  2.  Geben Sie auf der Seite **Programm für Gerät** Folgendes an:  
        ...
        -   **Kommentar:** Geben Sie bei Bedarf einen Kommentar für dieses Geräteprogramm mit maximal 127 Zeichen an.  

        -   **Downloadordner:** Geben Sie hier den Namen des Ordners auf dem Windows CE-Gerät an, in dem die Paketquelldateien gespeichert werden sollen. Der Standardwert ist **\Temp\\**.  
        ...
        -   **Befehlszeile von diesem Ordner ausführen:** Bei Auswahl dieser Option können Sie einen anderen Ordner angeben, von dem das Programm ausgeführt wird.  
 ...
 Das neue Paket und Programm wird im Arbeitsbereich **Softwarebibliothek** im Knoten **Pakete** angezeigt.  
...
4.  Wählen Sie im Assistenten zum Erstellen eines Pakets aus einer Definition auf der Seite **Paketdefinition** eine vorhandene Paketdefinitionsdatei aus, oder klicken Sie auf **Durchsuchen** , um eine neue Paketdefinitionsdatei zu öffnen. Nachdem Sie eine neue Paketdefinitionsdatei angegeben haben, wählen Sie sie in der Liste **Paketdefinition** aus, und klicken Sie dann auf **Weiter**.  

5.  Geben Sie auf der Seite **Quelldateien** Informationen zu ggf. erforderlichen Quelldateien für das Paket und Programm an, und klicken Sie auf **Weiter**.  

6.  Wenn für das Paket Quelldateien erforderlich sind, geben Sie auf der Seite **Quellordner** den Speicherort zum Abrufen der Quelldateien an, und klicken Sie auf **Weiter**.  

7.  Überprüfen Sie auf der Seite **Zusammenfassung** die auszuführenden Aktionen, und schließen Sie den Assistenten ab. Das neue Paket und Programm wird im Arbeitsbereich **Softwarebibliothek** im Knoten **Pakete** angezeigt.  
...
3.  Wählen Sie das Paket, das Sie bereitstellen möchten, und klicken Sie dann in der **Home** Registerkarte der **Bereitstellung** zu gruppieren, klicken Sie auf **Bereitstellen**.  

4.  Auf der **Allgemeine** Seite des Assistenten zum Bereitstellen von Software Geben Sie den Namen für das Paket und Programm, das Sie bereitstellen möchten, in der Sie das Paket und Programm sowie optionale Kommentare zur Bereitstellung bereitstellen möchten.  
...
5.  Klicken Sie auf der Seite **Inhalt** auf **Hinzufügen**, und wählen Sie anschließend die Verteilungspunkte oder Verteilungspunktgruppen aus, auf denen der diesem Paket und diesem Programm zugeordnete Inhalt bereitgestellt werden soll.  

6.  Wählen Sie auf der Seite **Bereitstellungseinstellungen** einen Zweck für diese Bereitstellung aus, und geben Sie Optionen für Aktivierungspakete und getaktete Verbindungen an:  
    ...
    -   Wählen Sie falls erforderlich die Option **Clients mit einer getakteten Internetverbindung dürfen den Inhalt nach dem Installationsstichtag herunterladen (Zusatzkosten können anfallen)** aus.  
...
7.  Konfigurieren Sie auf der Seite **Zeitplanung**, wann dieses Paket und dieses Programm bereitgestellt oder für Clientgeräte verfügbar gemacht werden.  
...
8.  Wenn der Zweck der Bereitstellung auf **erforderliche**, konfigurieren Sie das Verhalten beim erneuten Ausführen des Programms aus der **Verhalten beim erneuten Ausführen** Dropdown-Liste. Wählen Sie aus den folgenden Optionen:  

    |Verhalten beim erneuten Ausführen|Weitere Informationen|  
    |--------------------|----------------------|  
    |Bereitgestelltes Programm nie erneut ausführen|Das Programm wird nicht auf dem Client erneut ausgeführt werden, auch wenn die Anwendung ursprünglich nicht, oder die Programmdateien geändert werden.|  
    |Programm immer erneut ausführen|Das Programm wird immer auf dem Client erneut ausgeführt werden, wenn die Bereitstellung geplant ist, auch wenn das Programm bereits erfolgreich ausgeführt wurde. Dies kann nützlich sein, wenn Sie wiederholte Bereitstellungen verwenden, in denen das Programm, z. B. mit antivirus-Software aktualisiert wird.|  
    ...
    |Bei erfolgreichem vorherigen Versuch erneut ausführen|Das Programm wird erneut ausgeführt werden, nur dann, wenn er zuvor erfolgreich auf dem Client ausgeführt wurde. Dies ist nützlich, wenn sich wiederholende Ankündigungen verwendet werden, in denen das Programm regelmäßig aktualisiert wird und eine erfolgreiche Installation des jeweils vorherigen Updates erforderlich ist.|  

9. Geben Sie auf der Seite **Benutzerfreundlichkeit** die folgenden Informationen an:  
...
10. Geben Sie auf der Seite **Verteilungspunkte** die folgenden Informationen an:  
    ...
    -   **Freigeben von Inhalten für andere Clients im gleichen Subnetz zulassen** – wählen Sie diese Option zum Reduzieren der Auslastung des Netzwerks, dass Clients zum Herunterladen von Inhalt von anderen Clients im Netzwerk, die bereits heruntergeladen und zwischengespeichert und den Inhalt. Diese Option wird Windows BranchCache und oder höher auf Computern mit Windows Vista SP2 verwendet werden kann.  

    -   **Die Verwendung eines Fallbackquellpfads für den Inhalt durch Clients zulassen** : Wenn diese Option aktiviert ist, können Clients andere Verteilungspunkte in der Hierarchie nach erforderlichen Inhalten durchsuchen, wenn diese auf den angegebenen Verteilungspunkten oder in Verteilungspunktgruppen nicht verfügbar sind.  

11. Überprüfen Sie auf der Seite **Zusammenfassung** die auszuführenden Aktionen, und schließen Sie den Assistenten ab.  
...
> [!IMPORTANT]  
>  Wenn Sie die Option **Programm vom Verteilungspunkt ausführen** auf der Seite **Verteilungspunkte** des Assistenten zum Bereitstellen von Software konfiguriert haben, dürfen Sie die Option **Den Inhalt in diesem Paket in eine Paketfreigabe auf Verteilungspunkten kopieren**nicht deaktivieren, da das Paket dann nicht mehr von Verteilungspunkten aus ausgeführt werden kann.  

##  <a name="monitor-packages-and-programs"></a>Überwachen von Paketen und Programmen  
 Zum Überwachen von Paket- und Programmbereitstellungen verwenden Sie die gleichen Verfahren wie zur Überwachung von Anwendungen, wie in [Überwachen von Anwendungen](/sccm/apps/deploy-use/monitor-applications-from-the-console) beschrieben.  

 Pakete und Programme in umfassen darüber hinaus mehrere integrierte Berichte, mit deren Hilfe eine Überwachung der Daten zum Bereitstellungsstatus von Paketen und Programmen möglich ist. Diesen Berichten sind die Berichtskategorien **Softwareverteilung – Paket- und Programmbereitstellung** und **Softwareverteilung – Status der Paket- und Programmbereitstellung**zugeordnet.  
...
##  <a name="manage-packages-and-programs"></a>Verwalten von Paketen und Programmen  
 Erweitern Sie im Arbeitsbereich **Softwarebibliothek** den Knoten **Anwendungsverwaltung**, klicken Sie auf **Pakete**, wählen Sie das zu verwaltende Paket und dann einen Verwaltungstask aus der folgenden Tabelle aus:  

|Aufgabe|Weitere Informationen|  
|----------|----------------------|  
|**Datei für vorab bereitgestellten Inhalt erstellen**|Öffnet das vorab bereitgestellten Inhalt Datei-Assistent, mit dem Sie eine Datei erstellen, die den Paketinhalt enthält, der manuell in einem anderen Standort importiert werden kann. Dies bietet sich an, wenn die Bandbreite zwischen Standortserver und Verteilungspunkt eingeschränkt ist.|  
|**Programm erstellen**|Öffnet den Assistenten zum Erstellen von Programmen, mit dessen Hilfe Sie ein neues Programm für dieses Paket erstellen können.|  
|**Exportierenieren**|Öffnet den Assistenten zum Exportieren von Paketen, mit dessen Hilfe Sie das ausgewählte Paket und dessen Inhalt in eine Datei exportieren können.<br /><br /> Informationen zum Importieren von Paketen und Programmen finden Sie im Abschnitt „Erstellen von Paketen und Programmen“ in diesem Thema.|  
|**Bereitstellen**|Öffnet den Assistenten zum Bereitstellen von Software, mit dessen Hilfe Sie das ausgewählte Paket und Programm in einer Sammlung bereitstellen können. Weitere Informationen finden Sie unter [Bereitstellen von Paketen und Programmen](/sccm/apps/deploy-use/packages-and-programs#deploy-packages-and-programs) in diesem Thema.|  
|**Inhalt verteilen**|Öffnet den Assistenten Verteilung von Inhalt, mit dem Sie den Inhalt zu senden, der das Paket und Programm auf ausgewählten Verteilungspunkten oder Verteilungspunktgruppen zugeordnet ist.|  
...
##  <a name="about-the-package-definition-file-format"></a>Informationen zum Format von Paketdefinitionsdateien  
 Paketdefinitionsdateien sind Skripts, die Sie bei der automatischen Erstellung von Paketen und Programmen mit Configuration Manager unterstützen. Sie enthalten alle Informationen, die von Configuration Manager benötigt werden, um ein Paket und Programm, mit Ausnahme des Speicherorts der Paketquelldateien, zu erstellen. Jede Paketdefinitionsdatei ist eine ASCII- oder UTF-8-Textdatei im INI-Datei-Format enthält folgende Abschnitte beschrieben:  

###  <a name="pdf"></a>[PDF]  
 Dieser Abschnitt kennzeichnet die Datei als Paketdefinitionsdatei. Es enthält die folgende Informationen:  

-   **Version**: Dies gibt die Version des Paket-Dateiformats Definition, die von der Datei verwendet wird. Dies entspricht der Version von Systems Management Server (SMS) oder Configuration Manager, für die sie geschrieben wurde. Dieser Eintrag ist obligatorisch.  
...
-   **Icon**: Optional, die Datei, die die Symbol für dieses Paket enthält. Wenn angegeben, ersetzt dieses Symbol das Standardpaketsymbol in der Configuration Manager-Konsole.  
...
-   **Icon**: Gibt optional die Datei mit dem Symbol für dieses Programm an. Wenn angegeben, ersetzt dieses Symbol das Standardprogrammsymbol in der Configuration Manager-Konsole und wird bei der Ankündigung des Programms auf Clientcomputern angezeigt.  
...
-   **DependentProgram**: Gibt ein Programm in diesem Paket, das vor dem aktuellen Programm ausgeführt werden muss. Für diesen Eintrag wird folgendes Format verwendet: **DependentProgram**=<**Programmname>**, wobei **<Programmname\>** der Eintrag **Name** für das Programm in der Paketdefinitionsdatei ist. Lassen Sie den Eintrag leer, wenn keine abhängigen Programme vorhanden sind.  
